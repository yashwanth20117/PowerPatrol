    ...
    <div className="space-y-4">
      ...
      <Drawer open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        ...
        <DrawerContent className={`border-slate-700 max-h-[90vh] ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`}>
          <DrawerHeader>
            <DrawerTitle className={theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}>Add New Appliance</DrawerTitle>
            <DrawerDescription className={theme === 'dark' ? 'text-slate-400' : 'text-gray-600'}>
                  ...
                  {APPLIANCE_PRESETS.map((preset, index) => (
                    ...
                    >
                      <div className="text-left w-full">
                        <div className={`text-sm ${theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}`}>{preset.name}</div>
      ...
      <Card className={theme === 'dark' ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-gray-200'}>
        <CardHeader>
          <CardTitle className={`text-base ${theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}`}>My Devices</CardTitle>
          <CardDescription className={`text-sm ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'}`}>Tap to toggle on/off</CardDescription>
        </CardHeader>
        <CardContent>
          {appliances.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-3 gap-2 text-sm">
                      <div className={`text-center p-2 rounded ${theme === 'dark' ? 'bg-slate-900/50' : 'bg-gray-100'}`}>
                        <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'}`}>Power</p>
                        <p className={`text-sm ${theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}`}>{appliance.wattage}W</p>
                      </div>
                      <div className={`text-center p-2 rounded ${theme === 'dark' ? 'bg-slate-900/50' : 'bg-gray-100'}`}>
                        <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'}`}>Usage</p>
                        <p className={`text-sm ${theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}`}>{appliance.hoursPerDay}h</p>
                      </div>
                      <div className={`text-center p-2 rounded ${theme === 'dark' ? 'bg-slate-900/50' : 'bg-gray-100'}`}>
                        <p className={`text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-gray-600'}`}>Daily</p>
                        <p className={`text-sm ${theme === 'dark' ? 'text-slate-100' : 'text-gray-900'}`}>
